...
class Fuse_Down_5(nn.Module):

    def __init__(self, t2_model):
        super(Fuse_Down_5, self).__init__()
...
class nnUNetTrainer_Down5(nnUNetTrainer):
    ...
    def build_network_architecture( cls, plans_manager, dataset_json, configuration_manager, num_input_channels, enable_deep_supervision) -> nn.Module:
        ...
        t2_conv_block_stage_6 = t2_network.encoder.stages[6][0].convs[0]
        #only thing changed here is the input channels. DOuble what would normally be expected 
        setattr(t2_conv_block_stage_6, "conv", nn.Conv3d(640, 320, kernel_size=(3, 3, 3), stride=(1, 2, 2), padding=(1, 1, 1)))
        t2_conv_block_stage_6.all_modules[0] = t2_conv_block_stage_6.conv
        ...
        t2_decoder_conv_sixth_to_last_block = t2_network.decoder.stages[-6].convs[0]
        setattr(t2_decoder_conv_sixth_to_last_block, "conv", nn.Conv3d(960, 320, kernel_size=(3, 3, 3), stride=(1, 1, 1), padding=(1, 1, 1))) 
        t2_decoder_conv_sixth_to_last_block.all_modules[0] = t2_decoder_conv_sixth_to_last_block.conv


        # pdb.set_trace()

        # fuse the models 
        network = Fuse_Down_5(t2_model=t2_network)

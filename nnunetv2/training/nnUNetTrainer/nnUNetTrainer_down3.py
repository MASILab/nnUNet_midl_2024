...
class Fuse_Down_3(nn.Module):

    def __init__(self, t2_model):
        super(Fuse_Down_3, self).__init__()
...
class nnUNetTrainer_Down3(nnUNetTrainer):
    ...
    def build_network_architecture( cls, plans_manager, dataset_json, configuration_manager, num_input_channels, enable_deep_supervision) -> nn.Module:
        ...
        t2_conv_block_stage_4 = t2_network.encoder.stages[4][0].convs[0]
        #only thing changed here is the input channels. DOuble what would normally be expected 
        setattr(t2_conv_block_stage_4, "conv", nn.Conv3d(512, 320, kernel_size=(3, 3, 3), stride=(1, 2, 2), padding=(1, 1, 1))) # reassign to 1 input channel for a single modality
        t2_conv_block_stage_4.all_modules[0] = t2_conv_block_stage_4.conv
        ...
        # fuse the models 
        network = Fuse_Down_3(t2_model=t2_network)
